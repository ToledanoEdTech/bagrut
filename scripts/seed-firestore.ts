...
async function main() {
  ...
  for (const p of data.paths) {
    ...
    await adminDb.collection("examPaths").doc(pathId).update({ subjectIds });
  }
  ...
  const flexibleSubjects = await adminDb.collection("subjects").where("category", "==", "TRACK").get();
  for (const doc of flexibleSubjects.docs) {
