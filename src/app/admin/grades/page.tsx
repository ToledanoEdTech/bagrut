...
export default function GradesPage() {
  ...
  const loadStudentData = useCallback(async (studentId: string) => {
    ...
    const [gradesRes, pathSubjectsRes, mathRes, englishRes] = await Promise.all([
      fetch(`/api/grades?studentId=${studentId}`),
      fetch(`/api/subjects?pathId=${student.class.examPathId}`),
      fetch(`/api/subjects`).then((r) => r.json()).then((all: Subject[]) =>
        all.find((s) => s.name.includes("מתמטיקה") && s.units === student.mathUnits)
      ),
      fetch(`/api/subjects`).then((r) => r.json()).then((all: Subject[]) =>
        all.find((s) => s.name.includes("אנגלית") && s.units === student.englishUnits)
      ),
    ]);

    const gradesData = await gradesRes.json();
    setGrades(
      gradesData.map((g: { obligationId: string; score: number | null; status: string }) => ({
    ...
    const pathSubjects: Subject[] = await pathSubjectsRes.json();
    const allSubjects = [...pathSubjects];
    if (mathRes) allSubjects.push(mathRes);
    if (englishRes) allSubjects.push(englishRes);
        ...
        {selectedStudent && (
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-500">
            ...
            {selectedStudent.track && (
              <span>מגמה: {selectedStudent.track.name}</span>
            )}
          </div>
        )}
      </div>
