...
import { computeOutstandingBagrutForStudents } from "@/lib/outstanding-bagrut";
...
export async function GET() {
  ...
  const results = await computeOutstandingBagrutForStudents(scopedStudents);
  const candidates = results.filter((item) => item.outstandingBagrut.isCandidate);
  ...
  return NextResponse.json({
    ...
    byStudentId: Object.fromEntries(
      results.map((item) => [item.studentId, item.outstandingBagrut])
    ),
  });
}
